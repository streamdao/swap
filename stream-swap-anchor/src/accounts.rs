//! Accounts structs for StreamSwap.
...
pub struct WithdrawOne<'info> {
    ...
    /// StreamSwap `D` invariant requires both the base and quote reserves
    /// to determine how many tokens are paid out to users withdrawing from
    /// the swap.
    ///
    /// *For more info, see [stream_swap_client::state::SwapTokenInfo::reserves].*
...
/// Token accounts for the output of a StreamSwap instruction.
#[derive(Accounts, Clone)]
