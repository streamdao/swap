...
use chrono::prelude::*;
use fuzz::{
    native_account_data::NativeAccountData, native_stream_swap::NativeStreamSwap,
...
fn run_swaps(argv: Vec<SwapArgs>) {
    ...
    let mut token_a_account =
        stream_swap.create_token_a_account(user_account.clone(), INITIAL_USER_TOKEN_A_AMOUNT);
    let mut token_b_account =
        stream_swap.create_token_b_account(user_account.clone(), INITIAL_USER_TOKEN_B_AMOUNT);
