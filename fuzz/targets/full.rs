...
use fuzz::{
    ...
    native_stream_swap::{get_swap_state, NativeStreamSwap, TokenType},
...
use stream_swap::{
    curve::{StreamSwap, MAX_AMP, MIN_AMP},
    error::SwapError,
    fees::Fees,
    instruction::*,
};
...
fn run_actions(actions: Vec<Action>) {
    ...
    let mut stream_swap = NativeStreamSwap::new(
        Utc::now().timestamp(),
        amp_factor,
    ...
    for action in &actions {
        ...
        if let Some(pool_token_id) = pool_token_id {
            let account_pairs = (
                signing_account.clone(),
                stream_swap.create_pool_account(signing_account.clone()),
            );
            pool_accounts
                .entry(pool_token_id)
                .or_insert_with(|| account_pairs);
    ...
    let after_total_token_a = token_a_accounts
        ...
        .sum::<u64>()
        + get_token_balance(&stream_swap.token_a_account)
        + get_token_balance(&stream_swap.admin_fee_a_account);
    ...
    let after_total_token_b = token_b_accounts
        ...
        .sum::<u64>()
        + get_token_balance(&stream_swap.token_b_account)
        + get_token_balance(&stream_swap.admin_fee_b_account);
...
fn run_action(
    action: &Action,
    stream_swap: &mut NativeStreamSwap,
    ...
    let initial_mint_supply = get_mint_supply(&stream_swap.pool_mint_account);
    let initial_swap_state = get_swap_state(&stream_swap.swap_account);
    let initial_token_a_balance = get_token_balance(&stream_swap.token_a_account);
    let initial_token_b_balance = get_token_balance(&stream_swap.token_b_account);

    let initial_invariant = StreamSwap::new(
    ...
    let result = match action {
        Action::Swap {
            ...
            match trade_direction {
                TradeDirection::AtoB => stream_swap.swap_a_to_b(
                    Utc::now().timestamp(),
                ...
                TradeDirection::BtoA => stream_swap.swap_b_to_a(
                    Utc::now().timestamp(),
        ...
        Action::RampA { instruction_data } => {
            stream_swap.ramp_a(Utc::now().timestamp(), *instruction_data)
        }
        Action::StopRampA => stream_swap.stop_ramp_a(Utc::now().timestamp()),
    };

    let current_mint_supply = get_mint_supply(&stream_swap.pool_mint_account);
    let current_swap_state = get_swap_state(&stream_swap.swap_account);
    let current_token_a_balance = get_token_balance(&stream_swap.token_a_account);
    let current_token_b_balance = get_token_balance(&stream_swap.token_b_account);

    let current_invariant = StreamSwap::new(
