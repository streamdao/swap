...
use stream_swap::{fees::Fees, instruction::*, state::SwapInfo};
...
impl NativeStreamSwap {
    pub fn new(
        ...
        let mut swap_account = NativeAccountData::new(SwapInfo::LEN, stream_swap::id());
        let (authority_key, nonce) =
            Pubkey::find_program_address(&[&swap_account.key.to_bytes()[..]], &stream_swap::id());
