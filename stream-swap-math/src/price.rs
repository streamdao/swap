...
use crate::{bn::U192, curve::StreamSwap};

/// Utilities for calculating the virtual price of a Stream LP token.
///
/// This is especially useful for if you want to use a Stream LP token as collateral.
///
/// # Calculating liquidation value
///
/// To use a Stream LP token as collateral, you will need to fetch the prices
/// of both of the tokens in the pool and get the min of the two. Then,
/// use the [StreamSwap::calculate_virtual_price_of_pool_tokens] function to
...
pub struct StreamSwap {
    ...
    ///
    /// See [`StreamSwap::compute_amp_factor`].
...
impl From<&StreamSwap> for crate::curve::StreamSwap {
    fn from(swap: &StreamSwap) -> Self {
        crate::curve::StreamSwap::new(
...
impl StreamSwap {
    ...
    ///
    /// The virtual price in the StreamSwap algorithm is obtained through taking the invariance
    ...
    pub fn compute_d(&self) -> Option<U192> {
        let calculator = StreamSwap::from(self);
