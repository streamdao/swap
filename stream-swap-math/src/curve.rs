...
use stream_swap_client::{
    fees::Fees,
    solana_program::{clock::Clock, program_error::ProgramError, sysvar::Sysvar},
    state::SwapInfo,
};

/// Number of coins/tokens in a swap.
/// The StreamSwap only supports 2 tokens.
...
/// The [StreamSwap] invariant calculator.
///
...
/// # Resources:
///
/// - [Curve StreamSwap paper](https://curve.fi/files/streamswap-paper.pdf)
/// - [StreamSwap Python model](https://github.com/streamdao/stream-swap/blob/master/stream-swap-math/sim/simulation.py)
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
impl TryFrom<&SwapInfo> for StreamSwap {
    type Error = ProgramError;

    fn try_from(info: &SwapInfo) -> Result<Self, ProgramError> {
        Ok(StreamSwap::new_from_swap_info(
            info,
...
impl StreamSwap {
    /// Constructs a new [StreamSwap] from a [SwapInfo].
    pub fn new_from_swap_info(info: &SwapInfo, current_ts: i64) -> StreamSwap {
        StreamSwap::new(
    ...
    /// Constructs a new [StreamSwap] invariant calculator.
    pub fn new(
        initial_amp_factor: u64,
        target_amp_factor: u64,
        current_ts: i64,
    ...
    /// performing swaps. The lower it is, the closer the invariant is to the constant product[^streamswap].
    ///
    ...
    ///
    /// [^streamswap]: [StreamProtocol, "StreamSwap," 2021.](https://curve.fi/files/streamswap-paper.pdf)
    ...
    ///
    /// *For more info on reserves, see [stream_swap_client::state::SwapTokenInfo::reserves].*
...
mod tests {
    ...
    proptest! {
        ...
        fn test_swaps_does_not_result_in_more_tokens(
            ...
            let mut t = SwapTest { stream_swap: &stream_swap, swap_reserve_balance_a: MAX_TOKENS_IN, swap_reserve_balance_b: MAX_TOKENS_IN, user_token_balance_a: initial_user_token_a_amount, user_token_balance_b: initial_user_token_b_amount };
            ...
            for i in 0..ITERATIONS {
                ...
                assert!(before_balance_a + before_balance_b >= after_balance, "before_a: {}, before_b: {}, after_a: {}, after_b: {}, swap: {:?}", before_balance_a, before_balance_b, t.user_token_balance_a, t.user_token_balance_b, stream_swap);
            }
    ...
    proptest! {
        ...
        fn test_virtual_price_does_not_decrease_from_deposit(
            ...
            let start_ramp_ts = cmp::max(0, current_ts - MIN_RAMP_DURATION);
            let stop_ramp_ts = cmp::min(i64::MAX, current_ts + MIN_RAMP_DURATION);
            let invariant = StreamSwap::new(amp_factor, amp_factor, current_ts, start_ramp_ts, stop_ramp_ts);
